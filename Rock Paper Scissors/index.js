...
let loop = true;
let level = 0; //level for game
let points = 0; //points of game with this you can purchase items
let score = 0; //score similar to level this will just count how many time you won
let Cheat = false; //item of shop
let Revive = false; // another item of shop
// <!.. -----------  Assigining Variable for game end here -------------- ..!>
// <!.. -----------  While loop for game start here -------------- ..!>
while (loop) {
    let index = auto_number(2, 0); //assigning number between 0-2 with the help of number generator 
...
.x.x.x.x.x.x.x.x.x.x.x.x.x..x.x.x.x.x.x.x.x.x.x.x.x.x.x.x.x.x.x.
 ...
 \tPlayer name: ${chalk.bold.yellow(username)} \t|| \tGame Level:${chalk.bold.yellow(level)}
 \tCurrent points:${chalk.bold.yellow(points)}   \t|| \tPlayer score:${chalk.bold.yellow(score)}
...
`));
    ...
    let start_game = await inquirer.prompt([
        {
            name: "want_to_start",
            type: "list",
    ...
    // <!.. ----------- Shop start here -------------- ..!>
        ...
        let inner_loop = true; //inner loop for shop this will repeat 
        while (inner_loop) {
                ...
                let confirm = await inquirer.prompt([
                    {
                        name: "confirmation",
                        type: "list",
                        message: chalk.bold.yellow(`It will cost ${chalk.cyan(price)} Do you can to buy ${chalk.cyan(Shop_item)}?`),
                        choices: ["Yes", "No"],
                    },
                ]);
