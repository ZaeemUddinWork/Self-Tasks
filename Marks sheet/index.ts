...
//taking some data from user like name roll no 
  ...
  let total_obtail: number = marks.reduce((total, number) => total + number, 0);

  let total_marks: number = marks.length * 100;//total marks 

  let percentage: number = total_obtail / marks.length;//percentage formula to calculate percentage
